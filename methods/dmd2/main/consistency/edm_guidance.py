from main.consistency.edm_network import get_edm_network, sample_onestep
...
class EDMGuidance(nn.Module):
    def __init__(self, args, accelerator):
        ...
        self.real_unet = get_edm_network()        
        self.real_unet.load_state_dict(torch.load(args.teacher_model_path, map_location="cpu"), strict=True)
        ...
        if self.gan_classifier:
            self.cls_pred_branch = nn.Sequential(
                nn.Conv2d(in_channels=3, out_channels=768, kernel_size=4, stride=2, padding=1),  # 32x32 -> 16x16
    ...
    def compute_distribution_matching_loss(
        ...
        with torch.no_grad():
            ...
            pred_real_image = sample_onestep(self.real_unet, noisy_latents, labels, timestep_sigma, self.sigma_data)

            # pred_fake_image = self.fake_unet(noisy_latents, timestep_sigma, labels)
            pred_fake_image = sample_onestep(self.fake_unet, noisy_latents, labels, timestep_sigma, self.sigma_data)
    ...
    def compute_loss_fake(
        ...
        fake_x0_pred = sample_onestep(self.fake_unet, noisy_latents, labels, timestep_sigma, self.sigma_data)
    ...
    def compute_cls_logits(self, image, label):
        ...
        rep = sample_onestep(self.fake_unet, image, label, timestep_sigma, self.sigma_data, return_bottleneck=True).float() 
