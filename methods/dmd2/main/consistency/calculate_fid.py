...
def sample_for_fid(accelerator, current_model, args):
    ...
    while len(all_images_tensor) * args.batch_size * accelerator.num_processes < args.total_eval_samples:
        ...
        if args.consistency_model_path is not None:
            eval_images = sample_onestep_consistency(current_model, noise * args.conditioning_sigma, random_labels, timesteps[:random_labels.shape[0]] * args.conditioning_sigma)
        else:
            eval_images = sample_onestep(current_model, noise * args.conditioning_sigma, random_labels, timesteps[:random_labels.shape[0]] * args.conditioning_sigma)
