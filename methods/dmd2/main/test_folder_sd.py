...
from coco_eval.coco_evaluator import evaluate_model, compute_clip_score
...
def save_images_to_file(all_images, all_captions, save_dir_path, save_captions_path):
    ...
    for i, img in enumerate(all_images):
        ...
        image_pil.close()
    print('SAVING CAPTIONS TXT')
    with open(save_captions_path, "w", encoding="utf-8") as file:
        for line in all_captions[:len(all_images)]:
            print(line)
            file.write(line + "\n")
...
def evaluate():
    ...
    parser.add_argument("--save_captions_path", type=str)
    ...
    logger.info(f"tokenizer and dataloaders: {run.dir}", main_process_only=True)

    #while True:
    new_checkpoints = sorted(glob.glob(os.path.join(folder, "pytorch_model.bin")))
    ...
    print('GOOD')
    if args.save_dir_path and args.save_captions_path:
    
        save_dir_path = args.save_dir_path
        save_captions_path = args.save_captions_path
        
        print('SAVING IMAGES')
        save_images_to_file(data_dict['all_images'], data_dict['all_captions'], save_dir_path, save_captions_path)
